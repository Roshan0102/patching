...
def send_sns_notification(message, subject="WalletHR 24/7 Servers Patch Scan Report"):
...
def discover_24_7_servers():
    """Discovers running 24/7 web servers using tags."""
    logger.info("Starting Resource Discovery for 24/7 servers...")
    discovered = []
    try:
...
def main():
    ...
    # Step 1: Discover 24/7 Servers
    discovered_servers = discover_24_7_servers()

    if not discovered_servers:
        msg = "WalletHR 24/7 Servers Patch Scan Report:\n\nNo running 24/7 web servers discovered."
        logger.info(msg)
        send_sns_notification(msg)
        sys.exit(0)

    discovery_msg_lines = ["WalletHR 24/7 Servers Discovered:\n"]
    ...
    report_lines = ["WalletHR 24/7 Web Servers - Patch Scan Report\n"]
    ...
    if total_missing > 0:
        ...
        report_lines.append("\nScan Summary: All 24/7 web servers are fully compliant. No missing patches found.\n")
        report_lines.extend(missing_report)
