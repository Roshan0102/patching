...
# Logging setup
LOG_DIR = "24_7_servers_logs"
os.makedirs(LOG_DIR, exist_ok=True)
timestamp_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
log_filename = os.path.join(LOG_DIR, f"patch_24_7_{timestamp_str}.log")
...
logger = logging.getLogger("Patch247WebServers")
...
# =========================================================================
def send_sns_notification(message, subject="WalletHR 24/7 Servers Patching Update"):
    """
...
def discover_resources_step():
    ...
    try:
        ...
        msg_lines.append("\nNight Shutdown (Standby) Servers:")
        for tg_name, details in sorted(validated_groups.items()):
...
def scan_patches_step(groups_dict):
    logger.info("Starting Step 2: Scanning 24/7 servers for missing patches...")
    ...
    msg_lines = ["Step 2: Scan for Missing Patches Started\nInitiating SSM Patch Scan on 24/7 web servers:\n"]
    ...
    try:
        response = ssm.send_command(
            ...
            Comment="Automated pre-patching scan on 24/7 servers"
        )
        command_id = response["Command"]["CommandId"]
    except Exception as e:
        raise StepFailure("Step 2: Scan for Missing Patches", f"Failed to send SSM Scan command: {e}")
...
# =========================================================================
def start_standby_step(groups_dict):
    logger.info("Starting Step 3: Starting standby instances...")
    ...
    msg_lines = ["Step 3: Start Standby Instances Started\nInitiating startup of standby instances:\n"]
    for inst_id in instance_ids:
        msg_lines.append(f"- {id_to_name[inst_id]} ({inst_id})")
    send_sns_notification("\n".join(msg_lines))
    ...
    try:
        ...
        raise StepFailure("Step 3: Start Standby Instances", f"Failed to trigger instance start: {e}")
    ...
    if failed_instances:
        raise StepFailure("Step 3: Start Standby Instances", "\n".join(failed_instances))
    ...
    # Send success SNS
    msg_lines = ["Step 3: Start Standby Instances\nSuccessfully started and verified standby instances:\n"]
    for inst_id in instance_ids:
        msg_lines.append(f"- {id_to_name[inst_id]} ({inst_id})")
    send_sns_notification("\n".join(msg_lines))

# =========================================================================
# STEP 4: DEREGISTER THE 24/7 SERVERS FROM THEIR TARGET GROUPS
# =========================================================================
def deregister_targets_step(groups_dict):
    logger.info("Starting Step 4: Deregistering 24/7 servers from Target Groups...")
    ...
    for grp, details in groups_dict.items():
        ...
        results[inst["Name"]] = "Initiated"

    msg_lines = ["Step 4: Deregister 24/7 Servers Started\nInitiating deregistration from Target Groups:\n"]
    ...
    for tg_arn, inst_id, name in targets_to_deregister:
        try:
            ...
            raise StepFailure("Step 4: Deregister 24/7 Servers", f"Failed to deregister {name} ({inst_id}): {e}")
    ...
    if failed_draining:
        raise StepFailure("Step 4: Deregister 24/7 Servers", "\n".join(failed_draining))

    # Send Success SNS
    msg_lines = ["Step 4: Deregister 24/7 Servers\nSuccessfully deregistered and drained targets:\n"]
    for tg_arn, inst_id, name in targets_to_deregister:
        msg_lines.append(f"- {name} ({inst_id})")
    send_sns_notification("\n".join(msg_lines))
...
def create_backup_ami_step(groups_dict, prefix, step_num, step_name):
    ...
    for grp, details in groups_dict.items():
        start_msg_lines.append(f"- {details['24_7']['Name']} ({details['24_7']['InstanceId']})")
...
# =========================================================================
# STEP 6: PATCH THE 24/7 SERVERS
# =========================================================================
def patch_servers_step(groups_dict):
    logger.info("Starting Step 6: Patching 24/7 servers...")
    instance_ids = [details["24_7"]["InstanceId"] for details in groups_dict.values()]
    id_to_name = {details["24_7"]["InstanceId"]: details["24_7"]["Name"] for details in groups_dict.values()}
    
    msg_lines = ["Step 6: Patch 24/7 Servers Started\nInitiating SSM Patch Installation (NoReboot):\n"]
    for inst_id in instance_ids:
        msg_lines.append(f"- {id_to_name[inst_id]} ({inst_id})")
    send_sns_notification("\n".join(msg_lines))
    ...
    try:
        ...
        raise StepFailure("Step 6: Patch 24/7 Servers", f"Failed to trigger SSM patch execution: {e}")
    ...
    if failed_patching:
        raise StepFailure("Step 6: Patch 24/7 Servers", "Patch execution failed on instances:\n" + "\n".join(failed_patching))

    # Send Success SNS
    msg_lines = ["Step 6: Patch 24/7 Servers\nSuccessfully applied patches and rebooted instances:\n"]
    for inst_id in instance_ids:
        msg_lines.append(f"- {id_to_name[inst_id]} ({inst_id})")
    send_sns_notification("\n".join(msg_lines))

# =========================================================================
# STEP 8: RE-REGISTER THE 24/7 SERVERS WITH THEIR TARGET GROUPS
# =========================================================================
def register_targets_step(groups_dict):
    logger.info("Starting Step 8: Re-registering patched 24/7 servers back to ALB...")
    targets_to_register = []
    ...
    for tg_arn, inst_id, name in targets_to_register:
        try:
            ...
            raise StepFailure("Step 8: Re-register 24/7 Servers", f"Failed to register {name} ({inst_id}): {e}")
    ...
    if failed_health:
        raise StepFailure("Step 8: Re-register 24/7 Servers", "\n".join(failed_health))

    # Send Success SNS
    msg_lines = ["Step 8: Re-register 24/7 Servers\nSuccessfully registered and verified healthy targets:\n"]
    for tg_arn, inst_id, name in targets_to_register:
        msg_lines.append(f"- {name} ({inst_id})")
    send_sns_notification("\n".join(msg_lines))

# =========================================================================
# STEP 9: STOP THE STANDBY INSTANCES
# =========================================================================
def stop_standby_step(groups_dict):
    logger.info("Starting Step 9: Stopping standby instances...")
    instance_ids = [details["NightShutdown"]["InstanceId"] for details in groups_dict.values()]
    id_to_name = {details["NightShutdown"]["InstanceId"]: details["NightShutdown"]["Name"] for details in groups_dict.values()}

    msg_lines = ["Step 9: Stop Standby Instances Started\nInitiating cleanup / stopping standby instances:\n"]
    for inst_id in instance_ids:
        msg_lines.append(f"- {id_to_name[inst_id]} ({inst_id})")
    send_sns_notification("\n".join(msg_lines))
    ...
    try:
        ...
        raise StepFailure("Step 9: Stop Standby Instances", f"Failed to trigger stop: {e}")
    ...
    if failed_stop:
        raise StepFailure("Step 9: Stop Standby Instances", "\n".join(failed_stop))

    # Send Success SNS
    msg_lines = ["Step 9: Stop Standby Instances\nSuccessfully stopped standby instances:\n"]
    for inst_id in instance_ids:
        msg_lines.append(f"- {id_to_name[inst_id]} ({inst_id})")
    send_sns_notification("\n".join(msg_lines))
...
def send_final_notification(groups_dict):
    msg_lines = ["Step 10: Centralized Patching Complete\nAll patching steps successfully completed for the following 24/7 servers:\n"]
...
def main():
    logger.info("==================================================")
    logger.info("   WalletHR: Patching 24/7 Web Servers Script     ")
    logger.info("==================================================")

    workflow_name = "Patch 24/7 Web Servers"
    discovered_groups = {}
    ...
    try:
        ...
        for batch_index, batch_groups in enumerate(batches, start=1):
            ...
            # Step 3: Start the standby instances for this batch
            start_standby_step(batch_groups)

            # Step 4: Deregister 24/7 servers from their target groups for this batch
            ...
            # Step 6: Patch the 24/7 servers for this batch
            ...
            # Step 8: Re-register the 24/7 servers with their target groups for this batch
            register_targets_step(batch_groups)

            # Step 9: Stop the standby instances for this batch
            stop_standby_step(batch_groups)
        ...
        send_sns_notification(error_msg, subject="WalletHR 24/7 Servers Patching FAILURE")
        sys.exit(1)
    except Exception as e:
        error_msg = f"Workflow Error: {workflow_name} Failed.\n" \
